...
public class Util {
	...
	//static,non-static,파라미터존재여부,반환타입(o,x)
	//한자리수 값을 두자릿수 값으로 반환해주는 메서드
	...
	public void setToDates(String mDate) {//mdate-->화면으로부터 12자리 문자열을 받게될 파라미터
		//date형식:yyyy-MM-dd hh:mm:ss--->날짜형식으로 변환하려면 date 형식으로 모양을 바꿔줘야한다.
		//문자열을--->date타입으로 변환해야한다.
...
//		Integer.parseInt("a");//숫자형의 문자열을 정말 숫자형으로 변환하는 기능
	...
	if((dayOfWeek-1+i)%7==0){//토요일일면
   	...
   	public static String getCalView(List<CalDto> list, int i) {
   		...
   		String d=isTwo(i+"");//숫자1을 문자열 "01"로 바꿔줌
